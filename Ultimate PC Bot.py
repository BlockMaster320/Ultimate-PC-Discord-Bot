...
load_dotenv();
TOKEN = os.getenv("KVINTAB_TOKEN");                                                                                     #load token from .env file
...
channelPcName = 'photochallenges';
channelBiName = 'best-images';

emojiGeneralPointsName = 'PCicon_2'
emoji1pointName = 'heh';
emoji2pointName = 'baxCus';
emoji3pointName = 'vajcosW';
emojiTrophieBronze = 'heh';
emojiTrophieSilver = 'baxCus';
emojiTrophieGold = 'vajcosW';
...
async def on_ready():
    ...
    #Set Bot's Status
    await bot.change_presence(status = discord.Status.dnd, activity = discord.Game("VOLANT"));
    ...
    nextUpdateTime = lastUpdateDateBi + datetime.timedelta(days = biTimeRange + 7)
    zeroBeforeMinute = "0" * (nextUpdateTime.minute <= 9);
    print(f"Time until the next BI update: {(nextUpdateTime - currentTime).days} d {(nextUpdateTime - currentTime).seconds // 3600 + timeOffset} h {(nextUpdateTime - currentTime).seconds // 60} min [{nextUpdateTime.date()}, {nextUpdateTime.hour + timeOffset}:{zeroBeforeMinute}{nextUpdateTime.minute}].");
...
async def send_message_to_user(ctx, user: discord.Member, *, message):
    ...
    textFile = open("Kvinta B Bot DM History.txt", 'a');                                                                #save the message sent to the user to a .txt file
...
async def on_message(message):
    ...
    if (message.channel not in guild.channels) and (message.author != bot.user):
        ...
        textFile = open("Kvinta B Bot DM History.txt", 'a');                                                            #save the received message to a .txt file
...
async def show_help(ctx):
    ...
    helpEmbed = discord.Embed(title = "Ultimate PC Bot | Commands", colour = discord.Colour.blue());
...
def create_dataFile():
    ...
    lastUpdateDateBi = [datetime.datetime.utcnow() + datetime.timedelta(days = 1), 0];
    memberDict = {};
    imageNormalList = [];
    imagePcDict = {};
...
def create_embed(message, messagePoints, emojiGeneralPoints, title = None):
    ...
    imageEmbed.set_author(name = f"by {messageAuthor.name}", icon_url = messageAuthor.avatar_url);
    imageEmbed.set_image(url = message.attachments[0].url);
    zeroBeforeMinute = '0' * (messagePostDate.minute <= 9);
    imageEmbed.set_footer(text = f"posted {messagePostDate.date()}  {messagePostDate.hour}:{zeroBeforeMinute}{messagePostDate.minute}  UTC");
...
async def register_images(ctx, createDataFile = False):
    ...
    channelImages2 = discord.utils.get(guild.channels, name = 'u-wot-made-2');
    ...
    #Register the Images
    messageHistory1 = await channelImages.history(limit = None, after = lastUpdateDate, oldest_first = True).flatten();
    messageHistory2 = await channelImages2.history(limit = None, after = lastUpdateDate, oldest_first = True).flatten();
    messageHistory = messageHistory1 + messageHistory2;
    messageHistory.sort(key = lambda message: message.id);
    ...
    for message in messageHistory:
        if (message.attachments != []):
            ...
            messagePostDate = message.created_at;
            zeroBeforeMinute = "0" * (messagePostDate.minute <= 9);
            channelMark = 'U' * (message.channel.name == 'u-wot-made') + 'G' * (message.channel.name == 'u-wot-made-2')
            pcNumber = input(f"{channelMark} {messageAuthor.name} ({messagePostDate.hour + timeOffset}:{zeroBeforeMinute}{messagePostDate.minute}, {messagePostDate.day}/{messagePostDate.month}/{messagePostDate.year}): ");
            ...
            if (messageAuthor.id not in memberDict.keys()):                                                             #search for the MemberEdit object or create a new one
                memberDict[messageAuthor.id] = EditMember(messageAuthor.id, messageAuthor.name, messageAuthor.discriminator);
            ...
            galleryEmbed = create_embed(message, messagePoints, emojiGeneralPoints);                                    #send the image to the Gallery Channel
            ...
            if (pcNumber != ''):                                                                                        #send the image to the PhotoChallenges Channel
                pcEmbed = create_embed(message, messagePoints, emojiGeneralPoints, f"PhotoChallenge {pcNumber}");
...
async def update_bestImages(ctx):
    ...
    emojiGeneralPoints = discord.utils.get(guild.emojis, name = emojiGeneralPointsName);
    emojiTrohpieBronze = discord.utils.get(guild.emojis, name = emojiTrophieBronze);
    emojiTrohpieSilver = discord.utils.get(guild.emojis, name = emojiTrophieSilver);
    emojiTrohpieGold = discord.utils.get(guild.emojis, name = emojiTrophieGold);
    ...
    for message in messageList:                                                                                         #get top 5 (or more) images from the list and send them to the channelBi
        if (messageCount <= 5) or (lastMessagePoints == message[1]):
            if (messageCount == 1):
                trophy = emojiTrohpieGold;
            elif (messageCount == 2):
                trophy = emojiTrohpieSilver;
            elif (messageCount == 3):
                trophy = emojiTrohpieBronze;
            else:
                trophy = "";
...
async def edit_image(ctx, messageID: int, action):
    ...
    #Get the Image and Its Embeds
    embedGalleryMessage = None;
    embedPcMessage = None;
    imageMessage = None;
    imageMessagePoints = 0;
    ...
    if (action[0] in '123456789'):                                                                                      #add the image to EditMember's imagePcDict
        ...
        if (embedPcMessage is not None):                                                                                #when moving a PC image to a different PC (just update the existing PC embed)
            ...
            pcEmbed = create_embed(imageMessage, imageMessagePoints, emojiGeneralPoints, f"PhotoChallenge {pcNumber}");
            ...
            if (embedGalleryMessage is None):                                                                           #when adding the image to a new EditMember object (send a new gallery embed and PC embed)
                galleryEmbed = create_embed(imageMessage, imageMessagePoints, emojiGeneralPoints);
    ...
    #Create String Containing All the Images and PC Images
        ...
        for image in allImageList:
            try:
                ...
                allImageString += f"[image {imageCount}]({imageMessage.jump_url})\n";
            except:
